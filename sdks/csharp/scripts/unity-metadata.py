...
for path in sorted(root.rglob('*')):
    ...
    # Unity ignores folders ending in '~'. Keep metadata for their contents,
    # which become importable assets when a user imports a package sample.
    if path.is_dir() and path.name.endswith('~'): continue
